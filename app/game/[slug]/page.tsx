...
export default function GameDetailPage() {
  ...
  useEffect(() => {
    ...
    const foundGame = gameData.games.find((g) => g.name === gameUrl)
    setGame(foundGame)

    if (foundGame) {
      const suggested = gameData.games
        .filter((g) => g.category === foundGame.category && g.url !== foundGame.url)
        .slice(0, 4)
  ...
  const checkIfLiked = async (gameId: string) => {
    ...
    try {
      const { data } = await supabase.from("likes").select("id").eq("user_id", user.id).eq("game_id", gameId).single()
  ...
  const handleLike = async () => {
    ...
    if (!isSupabaseConfigured) {
      alert("Database not configured. Likes are not available in preview mode.")
      return
    }
    ...
    try {
      if (isLiked) {
        await supabase.from("likes").delete().eq("user_id", user.id).eq("game_id", game.url)
        setIsLiked(false)
      } else {
      ...
      <div className={`${isFullscreen ? "fixed inset-0 z-50 bg-black" : "p-4"}`}>
        <div className={`${isFullscreen ? "h-full" : "max-w-7xl mx-auto"}`}>
          ...
          <div className={`${isFullscreen ? "h-full" : "aspect-video"} bg-black rounded-lg overflow-hidden`}>
            <iframe src={game.iframes[0].src} className="w-full h-full border-0" allowFullScreen title={game.name} />
          </div>
        </div>
      </div>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-2">
                ...
                <h2 className="text-2xl font-bold text-white">More {game.category} Games</h2>
              </div>
              <div className="text-gray-400 text-sm">Discover similar games you might enjoy</div>
            </div>
              ...
              {suggestedGames.map((suggestedGame) => (
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-gray-400 text-sm capitalize mb-3">{suggestedGame.category}</p>
                    <div className="flex items-center justify-between">
                      <span className="text-purple-400 text-sm font-medium">Play Now</span>
